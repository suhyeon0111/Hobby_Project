package com.hoppy.app.login.api.controller.auth;

import com.hoppy.app.login.api.entity.auth.AuthReqModel;
...
public class AuthController {
    ...
    public ApiResponse login(HttpServletRequest request, HttpServletResponse response, @RequestBody AuthReqModel authReqModel) {
        ...
        Date now = new Date();
        AuthToken accessToken = tokenProvider.createAuthToken(userId, ((UserPrincipal) authentication.getPrincipal()).getRoleType().getCode(), new Date((now.getTime() + appProperties.getAuth().getTokenExpiry())));
    ...
    public ApiResponse refreshToken (HttpServletRequest request, HttpServletResponse response) {
        // access token 확인
        String accessToken = HeaderUtil.getAccessToken(request, "123");
        ...
        // refresh 토큰 유효 기간이 3일 이하로 남은 경우, refresh 토큰 갱신
